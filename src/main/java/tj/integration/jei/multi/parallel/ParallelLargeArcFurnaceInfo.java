...
public class ParallelLargeArcFurnaceInfo extends TJMultiblockInfoPage implements IParallelMultiblockInfoPage {
    ...
    public MultiblockShapeInfo getMatchingShapes(int extent) {
        ...
        return builder.aisle("~IMO~", "CCcCC", "CCcCC", "CCcCC", "~iEo~").where('S', this.getController(), WEST)
                    ...
                    .where('i', ChannelRegistry.OUTPUT_BUS,MetaTileEntities.ITEM_EXPORT_BUS[0], EAST)
                    .where('O', ChannelRegistry.INPUT_HATCH,MetaTileEntities.FLUID_IMPORT_HATCH[0], WEST)
                    .where('o', ChannelRegistry.OUTPUT_HATCH ,MetaTileEntities.FLUID_EXPORT_HATCH[0], EAST)
                    .build();
